...
- **连对阶梯**（仅逐词听写）：正常模式连对 6 →「还不错的天分」、9 →「相当好的天分」、12 →「了不起的天分」（+confetti），之后每 +6 重复「了不起」；错词复习模式单独用 3/4/5（5 后每 +3，2026-07-19 增加，短轮次下 6/9/12 触发不了）；答错静默归零（2026-07-18 由 3/4/5 上调，实测用户轻松十几连对）
- **结算盖章**：正确率 ≥80%「了不起」/ ≥70%「相当好」/ ≥60%「还不错」/ <60% 不盖章；仅生效于章节结算页；正确率 = 实际答对词数 / 总词数（2026-07-19 修正：断点续练/跳词导致的未作答不再算对）
- **开关**：听写设置「展示我的天分」，默认开；打字模式与连播卷面均不触发
- 「一般般的天分」素材封存未用（`manifests/天分/`）

---

## 错题本更新策略 · error-book-update-strategy（2026-07-19 已实现）

错题从「叠加」改「更新」口径。OpenSpec change：`openspec/changes/error-book-update-strategy`。

**最终方案**：

- **最新状态口径**：一个词（dict+word）是否还是错题，由其最新一条练习记录决定（`wrongCount>0` 在列 / `=0` 已掌握移出）；任何来源（正常听写、错词复习、连播卷面）的记录一视同仁，打字模式不区分（用户确认不咋用）
- **错题本视图切换**：「最新错题」（默认）/「全部错题」；全部视图中已掌握词打「✓已掌握」标记并弱化，错误次数照常显示排序，可勾选回炉；结算页「练习错题 (N)」同口径
- **累计错误次数永久保留**（不删记录），全部视图 + 次数排序作为"更新策略误清"的兜底
- **连播卷面判分写库**：「对答案」为已判分行写 `wordRecord`（对 0 / 错 1），重复判分只写判定变化的行
- 无新表、无 schema 变更；聚合时用 `word` 索引批量取候选词记录，内存判定最新状态
