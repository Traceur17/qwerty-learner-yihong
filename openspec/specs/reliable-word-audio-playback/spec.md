# reliable-word-audio-playback Specification

## Purpose

Make custom word audio playback reliable on the typing page via a shared player, playable-ready chapter preload, and visible failure feedback.

## Requirements

### Requirement: Chapter preload means playable readiness

For dictionaries with custom word audio, chapter preload SHALL treat a URL as loaded only after it is ready to play from the shared player memory cache. Byte source MAY be disk cache (same audio epoch) or network; readiness semantics remain decode-complete, and chapter entry remains blocked until ready or failed.
...
- **WHEN** all chapter custom URLs are already present in the current-epoch disk cache
