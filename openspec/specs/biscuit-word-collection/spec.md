...
Before or at save time, if a candidate word already exists in「小饼干罐」, the current dictionary, or a biscuit-series built-in dictionary, the system SHALL indicate which dictionary contains it. The user SHALL decide whether to still add it to「小饼干罐」(deselection or explicit confirm per product UI).
