...
During listen dictation mode, the typing page SHALL display only the word pronunciation audio and pronunciation icon. The system MUST NOT display phonetic symbols, translations, per-letter grids, previous/next word previews, or Tab-to-reveal-answer tooltips.

#### Scenario: UI elements hidden in listen dictation mode

- **WHEN** listen dictation mode is active and user is practicing
- **THEN** phonetic display is hidden
- **AND** translation display is hidden
- **AND** per-letter Letter component grid is hidden
- **AND** previous/next word preview is hidden
